...
impl LowerCtx<'_> {
    ...
    pub(crate) fn is_error_return(&self, values: &[HirExpr]) -> bool {
        if result_ok_err(&self.func_return_type).is_none() {
            return false;
        }
        match values.len() {
            0 => false,
            1 => self.expr_is_result_err(&values[0]),
            _ => false,
        }
    }
    ...
    pub(crate) fn lower_try_result(
        ...
        let base = self.lower_expr(inner, None, symbols)?;
    ...
    pub(crate) fn lower_expr(
        ...
        match &expr.kind {
            ...
            HirExprKind::Try { expr: inner } => {
                if result_ok_err(&inner.ty).is_some() {
                    self.lower_try_result(inner, target, expr.ty.clone(), symbols)
                } else if is_option_type(&inner.ty) || matches!(inner.ty, ArType::Nullable(_)) {
