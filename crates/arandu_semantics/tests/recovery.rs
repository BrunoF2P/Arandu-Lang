...
fn test_recovery_lexer() {
    ...
    assert!(!lexed_char.diagnostics.is_empty(), "invalid_char should have diagnostics");
    // Verify we still got tokens (meaning it recovered and continued tokenizing)
    assert!(!lexed_char.tokens.is_empty(), "should recover and produce tokens");
    ...
    assert!(!lexed_str.diagnostics.is_empty(), "unterminated_string should have diagnostics");
    assert!(!lexed_str.tokens.is_empty(), "should recover and produce tokens");
    ...
    assert!(!lexed_esc.diagnostics.is_empty(), "invalid_escape should have diagnostics");
    assert!(!lexed_esc.tokens.is_empty(), "should recover and produce tokens");
}
...
fn test_recovery_parser() {
    ...
    assert!(!output_rparen.diagnostics.is_empty(), "missing_rparen should have diagnostics");
    // Should have parsed some declarations / function
    assert!(!output_rparen.program.decls.is_empty(), "should have parsed declarations");
    ...
    assert!(!output_rbrace.diagnostics.is_empty(), "missing_rbrace should have diagnostics");
    assert!(!output_rbrace.program.decls.is_empty(), "should have parsed declarations");
    ...
    assert!(!output_stmt.diagnostics.is_empty(), "invalid_statement should have diagnostics");
    assert!(!output_stmt.program.decls.is_empty(), "should have parsed declarations");
    ...
    assert!(!output_expr.diagnostics.is_empty(), "invalid_expr should have diagnostics");
    assert!(!output_expr.program.decls.is_empty(), "should have parsed declarations");
}
...
fn test_recovery_name_resolution() {
    ...
    assert!(!res_val.diagnostics.is_empty(), "undefined value should have diagnostics");
    let codes: Vec<_> = res_val.diagnostics.iter().map(|d| d.code).collect();
    assert!(codes.contains(&arandu_semantics::DiagCode::N001UndefinedValue), "should report undefined value");
    // Verify Levenshtein suggestion hint
    let diag = res_val.diagnostics.iter().find(|d| d.code == arandu_semantics::DiagCode::N001UndefinedValue).unwrap();
    assert!(
        diag.hints.iter().any(|h| h.contains("my_variable")),
        "should suggest 'my_variable' in hints, got: {:?}", diag.hints
    );
    ...
    assert!(!res_ty.diagnostics.is_empty(), "undefined type should have diagnostics");
    let codes_ty: Vec<_> = res_ty.diagnostics.iter().map(|d| d.code).collect();
    assert!(codes_ty.contains(&arandu_semantics::DiagCode::N002UndefinedType), "should report undefined type");
    let diag_ty = res_ty.diagnostics.iter().find(|d| d.code == arandu_semantics::DiagCode::N002UndefinedType).unwrap();
    assert!(
        diag_ty.hints.iter().any(|h| h.contains("Person")),
        "should suggest 'Person' in hints, got: {:?}", diag_ty.hints
    );
}
