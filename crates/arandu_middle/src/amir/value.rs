...
pub enum AmirRvalue {
    ...
    /// F2.3.runtime: load payload through a gen ref (traps on generation mismatch).
    GenGet {
        gen_ref: AmirOperand,
    },
    /// F2.3.runtime: remove/recycle slot; returns payload (or zero) and invalidates the ref.
    GenRemove {
        gen_ref: AmirOperand,
    },
